...
from tkinter import messagebox, Button, Entry, Label, Checkbutton, Frame, Toplevel, OptionMenu, Radiobutton, Text, Scrollbar, StringVar, IntVar
...
class ToggleFrame(Frame):
    ...
    def __init__(self, parent):
        ...
        self.space.grid(column=2, row=0, pady=0, padx=spacepad, sticky='nsew')
        # Checkbutton to toggle the frame
        self.togButton = Checkbutton(self.show_frame, text='Show Options', command=self.tog_options, variable=self.show,
                                     fg='black', bg=gray, bd=4, font=('Arial', basefont), justify='left')
        self.togButton.grid(column=3, row=0, pady=0, padx=xpad, sticky='e')
        # Checkbutton to give user prompts after each action is completed
        self.prompt = IntVar()
        self.prompt.set(0)
        self.promptButton = Checkbutton(self.show_frame, text='Prompt after Each Action?', variable=self.prompt,
                                        fg='black', bg=gray, bd=4, font=('Arial', basefont), justify='left')
        self.promptButton.grid(column=4, row=0, pady=0, padx=xpad, sticky='e')
...
class ObjFormatter:
    ...
    def __init__(self, root):
        ...
        labl1 = tk.Label(frame1, text='Folder of\nItems:')
        ...
        browse1 = tk.Button(frame1, text='Browse', command=lambda: self.ask_folder(itemfolder, processfolder))
        ...
        self.e1 = tk.Entry(frame1, width=50, textvariable=itemfolder)
        ...
        '''
        # Create a StringVar for the processing folder
        processfolder = StringVar(frame1)
        labl2 = tk.Label(frame1, text='Processing\nFolder:')
        labl2.configure(fg='black', bg=blazegold, bd=0, font=('Arial', basefont), height=2, width=9, justify='center')
        labl2.grid(column=0, row=0, pady=5, padx=5, sticky='e')
        browse2 = tk.Button(frame1, text='Browse', command=lambda: self.ask_folder(processfolder))
        browse2.configure(bg=smoke, fg='black', highlightbackground=uabgreen, font=('Arial', entryfont))
        browse2.grid(column=2, row=0, pady=5, padx=5, sticky='w')
        self.e2 = tk.Entry(frame1, width=50, textvariable=processfolder)
        self.e2.configure(bg=gray, fg='black', relief='sunken', bd=2, font=('Arial', entryfont + 2), justify='left')
        self.e2.grid(column=1, row=0, pady=5, padx=0, sticky='w')

        # Create a StringVar for the CSV file (to be selected by the user)
        csvfile = StringVar(frame1)
        labl3 = tk.Label(frame1, text='CSV File:')
        ...
        browse3 = tk.Button(frame1, text='Browse', command=lambda: self.ask_file(csvfile, processfolder))
        browse3.configure(bg=smoke, fg='black', highlightbackground=uabgreen, font=('Arial', entryfont), relief='raised')
        browse3.grid(column=2, row=2, pady=5, padx=5, sticky='w')
        self.e3 = tk.Entry(frame1, width=50, textvariable=csvfile)
    ...
    def ask_folder(self, foname):
        ...
        selected_folder = path.abspath(
            filedialog.askdirectory(initialdir=startfolder1, title='Select the Processing Folder')
        )

        if selected_folder:
            foname.set(selected_folder)

        return foname


    def ask_file(self, fname, processfolder_var=None):
        startfolder2 = self.user_home()
        
        # Append the \csv_loaders subdirectory to the process folder if processfolder_var is provided
        if processfolder_var and processfolder_var.get():
            csv_loader_dir = path.join(processfolder_var.get(), 'csv_loaders')
        else:
            csv_loader_dir = startfolder2
        
        selected_file = path.abspath(filedialog.askopenfilename(initialdir=csv_loader_dir, title='Select the master CSV File', filetypes=[("CSV files", "*.csv")]))
        
        if selected_file:
            fname.set(selected_file)  # Set the selected file in the CSV field
    ...
    def run_bagit(self, bagsdir, storage_opt, moreopts4):
        """ Bags all objects in a single directory, according to APTrust BagIt profile """
        validbags = 0
        totalbags = 0
        if not storage_opt in ['Standard', 'Glacier-Deep-OH']:
            messagebox.showwarning(message=f'APT Storage Option \"{storage_opt}\" is not a valid option.\nStopping...')
            runnext4 = False
            return runnext4
        # Defining MData tags
        bag_info = {
    ...
    def run_procs(self, root, frame2):
        ...
        if procdir == "":
            messagebox.showwarning(message="You must first select a Processing folder.")
            return

        if not path.exists(procdir):
            messagebox.showwarning(message=f"Processing folder:\n'{procdir}'\nnot found.")
            return

        itemsdir = path.join(procdir, "ready_to_package")

        if not path.exists(itemsdir):
            messagebox.showwarning(
                message=f"No 'ready_to_package' folder found inside:\n{procdir}"
            )
            return
        ...
        self.apt_storage = frame2.aptStorage.get()
        nselect = 0
        ...
        if bagit == 1:
            nselect -= 1
            storage = self.apt_storage
            self.run_bagit(itemsdir, storage, nselect)
            if runnext == False:
                return
        # Run Tar
        if tar == 1:
